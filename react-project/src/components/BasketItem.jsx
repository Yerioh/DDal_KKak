...
const Basketitems = ({ items, index, sessionCheck, session }) => {
  ...
  const color_filter = COLOR.filter(
    (item) => item.COLOR_CODE === items.PROD_COLOR
  );
  ...
  const changeItemToCart = () => {
    ...
    let changeCartItem = {
      ...
      PROD_COLOR: {
        COLOR_CODE: `${items.PROD_COLOR.COLOR_CODE}`,
        COLOR_NAME: `${items.PROD_COLOR.COLOR_NAME}`,
      }, // 상품 색상
  ...
  const changebuyItem = () => {
    ...
    // 업데이트된 주문 데이터를 다시 JSON 형태로 변환하여 저장
    ...
    <div className="basket-goods-list">
      ...
      <div className="second-box">
        ...
        <div className="txt-info" style={{ width: "60%" }}>
          ...
          <div style={{ height: "50%" }}>{items.PROD_NAME}</div>
          <div style={{ height: "50%" }}>
            ...
            <button
              style={{
                ...
                width: "20px",
                height: "20px",
                backgroundColor: `${items.PROD_COLOR.COLOR_CODE}`,
              }}
            ></button>
          </div>
        </div>
      ...
      <div className="count-box">
          ...
          <div>
            <select onChange={sizeChange} name="size" id="">
              {prd_size_filter.map((size) => {
                return size.SIZE_NAME == items.PROD_SIZE ? (
                  <option value={size.SIZE_NAME} selected>
                    {size.SIZE_NAME}{" "}
                  </option>
                ) : (
                  <option value={size.SIZE_NAME} defaultValue={items.PROD_SIZE}>
                    {size.SIZE_NAME}{" "}
                  </option>
                );
              })}
        ...
        <div style={{ height: "50%", display: "grid", placeitems: "center" }}>
          ...
          >
            <button className="count-btn" onClick={subtract}>
              -
            </button>
            <h6 style={{ margin: "4px 5px" }}>{count}</h6>
            <button className="count-btn" onClick={addtion}>
              +
            </button>
          </div>
        </div>
