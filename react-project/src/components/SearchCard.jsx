...
const SearchCard = ({ data, index }) => {
  ...
  // 이미지 좋아요 클릭 여부 체크
  useEffect(()=>{
    axios.post('/imgCreate/likeCheck', {id : useId, imgId : data.IMG_ID})
      .then(res=>{
        setLikeBtn(res.data)
      })
  },[])
  ...
  return (
    ...
    >
      <Card.Img variant="top" src={`${process.env.REACT_APP_AWS_BUCKET_URL}/${data.IMG_URL}`}  onClick={handleShareImg}/>
      <Card.Body>
        {likeBtn ? (
